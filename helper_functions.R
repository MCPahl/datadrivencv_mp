...
\n\n\n"
    ...
    if(section_id == "Publications"){
        df <- df %>% 
            mutate(description_bullets = str_replace(description_bullets, "Matthew Pahl", "\\*\\*Matthew Pahl\\*\\*")) %>% 
            mutate(description_bullets = str_replace(description_bullets, "Pahl, MC", "\\*\\*Pahl, MC\\*\\*")) %>% 
            mutate(description_bullets = str_replace(description_bullets, "Pahl MC", "\\*\\*Pahl MC\\*\\*")) %>% 
            mutate(description_bullets = str_replace(description_bullets, "Matthew C Pahl", "\\*\\*Matthew C Pahl\\*\\*"))
    }
